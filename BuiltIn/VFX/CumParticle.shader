	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float localwave42 = ( 0.0 );
				float2 texCoord36 = packedInput.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float2 uv42 = texCoord36;
				float thickRange42 = 0.3;
				float thickWaveSpd42 = 7.5;
				float3 normal42 = float3( 0,0,0 );
				float visible42 = 0;
				{
				float thickWave = uv42.x*thickWaveSpd42;
				float curThick = 1.0-(sin(thickWave)*0.5+0.5)*thickRange42;
				visible42 = curThick - (abs(uv42.y-0.5)*2.0);
				float acrossAng = ((((1.0-uv42.y)-0.5)/curThick)+0.5)*3.14159;
				float xWaveCos = cos(thickWave)*thickRange42;
				float xWaveSin = ((sin(thickWave)*0.5)+0.5)*thickRange42 + (1.0-thickRange42);
				normal42 = float3(
				xWaveCos,
				...
				float3 normalizeResult7_g4 = normalize( ( _WorldSpaceCameraPos - ase_worldPos ) );
				float3 normalizeResult27_g5 = normalize( normalizeResult7_g4 );
				float3 viewToWorldDir6_g4 = normalize( mul( UNITY_MATRIX_I_V, float4( float3(0,1,0), 0 ) ).xyz );
				float3 normalizeResult31_g5 = normalize( viewToWorldDir6_g4 );
				float3 normalizeResult29_g5 = normalize( cross( normalizeResult27_g5 , normalizeResult31_g5 ) );
				...
				float3 tanNormal8_g4 = normal42;
				float3 worldNormal8_g4 = float3(dot(tanToWorld0,tanNormal8_g4), dot(tanToWorld1,tanNormal8_g4), dot(tanToWorld2,tanNormal8_g4));
				float dotResult10_g4 = dot( normalizeResult29_g5 , worldNormal8_g4 );
				float dotResult11_g4 = dot( cross( normalizeResult29_g5 , normalizeResult27_g5 ) , worldNormal8_g4 );
				float2 appendResult12_g4 = (float2(dotResult10_g4 , dotResult11_g4));
				
				surfaceDescription.BaseColor = _Color1.rgb;
				surfaceDescription.Normal = normal42;
				...
				surfaceDescription.Emission = ( ( _Color1 * 1.0 ) + tex2Dlod( _MatCap, float4( ( ( appendResult12_g4 * float2( 0.5,0.5 ) ) + float2( 0.5,0.5 ) ), 0, 0.0) ) ).rgb;
				surfaceDescription.Smoothness = 0.9;
				surfaceDescription.Occlusion = 1;
				surfaceDescription.Alpha = visible42;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float localwave42 = ( 0.0 );
				float2 texCoord36 = packedInput.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
				float2 uv42 = texCoord36;
				float thickRange42 = 0.3;
				float thickWaveSpd42 = 7.5;
				float3 normal42 = float3( 0,0,0 );
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float localwave42 = ( 0.0 );
				float2 texCoord36 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float2 uv42 = texCoord36;
				float thickRange42 = 0.3;
				float thickWaveSpd42 = 7.5;
				float3 normal42 = float3( 0,0,0 );
...
WireConnection;45;1;42;5
WireConnection;45;4;43;0
WireConnection;45;6;40;0
WireConnection;45;7;4;0
WireConnection;45;9;42;6
WireConnection;45;10;43;0
ASEEND*/
//CHKSM=CC065D51B59B4C26682D1C7C8568C06DD060E083