...
public class OrbitCameraCharacterBaseConfiguration : OrbitCameraConfiguration {
    ...
    [SerializeField]
    private OrbitCameraPivotBase ballPivot;
    [SerializeField]
    ...
    public override OrbitCameraData GetData(Camera cam) {
        ...
        OrbitCameraData ballData = ballPivot.GetData(cam);
        ...
        float forwardBack = (Vector3.Dot(character.GetFacingDirection()*Vector3.forward, rot * Vector3.forward)+1f)/2f;
        ...
        OrbitCameraData indulgenceCamera = OrbitCameraData.Lerp(ballsDickLerp, buttData, forwardBack);
